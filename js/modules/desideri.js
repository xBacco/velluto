...
let qpopBusy = false;
// Quick-add popover ancorato a un elemento della nav. Salva direttamente; il caller
// passa client/me esplicitamente perché può essere invocato anche prima che la tab
// Fantasie sia stata visitata (quindi ctx non è ancora popolato).
export function openQuickAdd({ client, me, anchorEl, onSaved }) {
  if (qpopBusy) return;
  qpopBusy = true;

  const scrim = mk('div', 'qpop-scrim');
  const pop = mk('div', 'qpop');
  const lbl = mk('div', 'qpop-lbl', '🔥 Nuova fantasia');
  const inp = mk('input', 'qpop-input'); inp.placeholder = 'Scrivi velocemente…';
  const row = mk('div', 'qpop-row');
  const btnX = mk('button', 'btn sm ghost', 'Annulla');
  const btnAdd = mk('button', 'btn sm gold', 'Aggiungi'); btnAdd.disabled = true;
  add(row, btnX, btnAdd);
  add(pop, lbl, inp, row);
  ...
  // posiziona il popover sopra il tab ancora, con freccia centrata sull'ancora
  const r = anchorEl.getBoundingClientRect();
  const W = window.innerWidth;
  const POP_W = Math.min(300, W - 24);
  pop.style.width = POP_W + 'px';
  const center = r.left + r.width / 2;
  let left = Math.round(center - POP_W / 2);
  left = Math.max(12, Math.min(left, W - POP_W - 12));
  pop.style.left = left + 'px';
  pop.style.bottom = Math.round(window.innerHeight - r.top + 12) + 'px';
  pop.style.setProperty('--arrow-x', Math.round(center - left) + 'px');

  requestAnimationFrame(() => { scrim.classList.add('on'); pop.classList.add('on'); });
  setTimeout(() => inp.focus(), 220);

  let closed = false;
  function close() {
    if (closed) return; closed = true;
    scrim.classList.remove('on'); pop.classList.remove('on');
    setTimeout(() => { scrim.remove(); pop.remove(); qpopBusy = false; }, 260);
  }
  scrim.onclick = close;
  btnX.onclick = close;
  inp.addEventListener('input', () => { btnAdd.disabled = !inp.value.trim(); });
  async function submit() {
    const v = inp.value.trim(); if (!v) return;
    btnAdd.disabled = true;
    try {
      await addDesiderio(client, { couple_id: me.couple_id, autore_id: me.id, testo: v, categoria: '' });
      close();
      toast('✓ Aggiunta a Fantasie');
      if (onSaved) onSaved();
    } catch (e) {
